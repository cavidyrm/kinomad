...
> **Deploying? Read `deploy/DEPLOY.md` first.** It contains the corrected `Dockerfile` and
> `nginx.conf` for the `cavidyrm/kinomad` pipeline, and the post-deploy checks.

### What broke on the last deploy

`KMAPI is not defined`, booking modal empty. The cause was **not** the page code — the repo's
`Dockerfile` copied files with a per-file whitelist and `km-api.js` was never added to it, so
the image shipped without the script. The request then hit nginx's SPA fallback
(`try_files … /index.html`), which returns HTML with a **200**, so the browser tried to parse a
web page as JavaScript and the global was never defined. `Kinomad Privacy.dc.html` was missing
from the same whitelist and had been 404ing since it was added.

`deploy/Dockerfile` copies by glob and asserts every required file exists, so the build fails
instead of the site. `deploy/nginx.conf` drops the HTML fallback for assets, so a missing file
returns an honest 404 rather than disguising itself as a script error.

Independently: a **relative** `./km-api.js` resolves against the current route, so it would also
404 on a nested URL (`/works/aurelia` asking for `/works/km-api.js`). All three pages that need
it now load it relatively and then retry from `/km-api.js` if the global is still missing. The
same hazard applies to `assets/…` — once per-slug URLs are live, switch the pages to
root-absolute paths or add `<base href="/">`.

**If the client is genuinely unreachable**, each page now fails on purpose rather than blanking:

- **Landing** — the scheduler falls back to the published studio hours, hides the busy feed, and
  refuses the final submit with a message pointing at the studio email. The page itself is unaffected.
- **CRM** — a *Console failed to load* screen naming `km-api.js` and the path fix, with Reload.
- **CRM sign in** — an inline notice above the form; the submit is blocked rather than hanging.
The prototype filenames (`Kinomad Landing.dc.html`) are **artifacts of the design tool, not the
intended URLs**. The public routing is now defined in one place — `km-routes.js` on the client,
`deploy/nginx.conf` on the server — and the pages rewrite their own links to match when served
from a host:
...
| `/` | Landing |
| `/works` | Works index |
| `/website/<slug>`, `/brand/<slug>`, `/motion/<slug>` | project detail, by type |
| `/privacy` | Privacy notice |
| `/admin` | CRM |
| `/admin/login` | CRM sign in |

The type prefix on detail pages is what lets a static server pick the right template; once the
backend renders project pages, collapse them to `/works/<slug>` and 301 the old form. Slug
reads accept both `?project=<slug>` and the path segment, so nothing breaks in between.
...
- `km-api.js` — the prototype's stand-in API (see above)
- `km-routes.js` — the canonical route table; rewrites the design-tool filenames to clean URLs when served from a host, and reads project slugs from the path
- `deploy/` — `Dockerfile`, `nginx.conf` and `DEPLOY.md`: drop-in replacements for the repo's build, plus the fix for the failed deploy
...
- Deploy fixed: the image was missing `km-api.js` and the privacy page; the build now copies by glob and verifies its own contents (`deploy/`).
